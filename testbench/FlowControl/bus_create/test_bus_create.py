...
sim_in1 = np.loadtxt(testdir/'sim_in1.csv', dtype=int).tolist()
sim_in2 = np.loadtxt(testdir/'sim_in2.csv', dtype=int).tolist()
expected_results = np.loadtxt(testdir/'sim_out.csv', dtype=int).tolist()

@cocotb.test()
async def bus_create_test(dut):
    """test bus_create """
    ninputs = int(dut.NBITS.value)
    nbits = int(dut.NINPUTS.value)
